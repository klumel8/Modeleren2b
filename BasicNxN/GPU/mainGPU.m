...
%actual mass.
MassCPU = linspace(1,N,N) * defaultMass/10;
MassCPU(1) = 1000*defaultMass;
MassCPU = reshape(MassCPU,[1,N]); %Used for the matrix multiplication in fo
Mass = gpuArray(MassCPU);
...
posCPU = gather(pos)
for i=1:N
    xPos(i,:) = posCPU(3*(i-1)+1,:);
    yPos(i,:) = posCPU(3*(i-1)+2,:);
    zPos(i,:) = posCPU(3*(i-1)+3,:);
end
