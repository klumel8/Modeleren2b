...
if level_of_awesomeness == 7  %Initialise leapfrog acceleration
...
for t = 0:dt:T
    ...
    if max(max(c)) > 0
       ...
       goodPoints = Mass~=0; %Check what points are the ones to continue. 
       %These will be used to redefine the usefull variables to only
       %include these "Good points"
       
       if level_of_awesomeness == 7 %Update accelration for collisions
           a(:,indices(1,:)) = (M(:,indices(1,:)).*a(:,indices(1,:)) + M(:,indices(2,:)).*a(:,indices(2,:)))./(M(:,indices(1,:))+M(:,indices(2,:)));
           a(:,indices(2,:)) = 0;
           a = a(:,goodPoints);
       end
       
       v = v(:,goodPoints);
       Mass = Mass(goodPoints);
       p = p(:,goodPoints);
       
       
       N = numel(Mass); %N = number of good points
